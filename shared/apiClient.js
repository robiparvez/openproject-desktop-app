...
class ApiClient {
  ...
  async checkExistingWorkPackageBySubject(projectId, subject) {
    const filters = JSON.stringify([
      ...
      { subject: { operator: '~', values: [subject.substring(0, 50)] } },
    ]);
    ...
    return workPackages.find(wp =>
      wp.subject.toLowerCase() === subject.toLowerCase()
    ) || null;
  }
  ...
  async checkExistingTimeEntries(workPackageId, spentOn) {
    const filters = JSON.stringify([
      { work_package: { operator: '=', values: [String(workPackageId)] } },
      { spent_on: { operator: '=d', values: [spentOn] } },
    ]);
