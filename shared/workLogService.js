...
async function processWorkLog(apiClient, timeline, onProgress) {
  ...
  for (const entry of timeline.entries) {
    try {
      ...
      if (!workPackageId) {
        const existing = await apiClient.checkExistingWorkPackageBySubject(
          entry.projectId,
          entry.subject
        );
        ...
        if (existing) {
          ...
          const newWp = await apiClient.createWorkPackage(
            entry.projectId,
            entry.subject
          );
      ...
      const existingTimeEntries = await apiClient.checkExistingTimeEntries(
        workPackageId,
        timeline.isoDate
      );
